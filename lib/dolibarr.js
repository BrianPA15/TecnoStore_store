...
async function createProduct(db, product) {
    ...
    // Usamos el SKU original sin romper sus caracteres especiales
    ...
    // CHECK EXISTENCIA ERP
    // =========================
    // Eliminamos sqlfilters conflictivos y buscamos directamente por la referencia limpia
    const search = await request(
        settings,
        'GET',
        `products?ref=${encodeURIComponent(safeSku)}&limit=1`
    );
    ...
    // =========================
    // CREACIÓN CON CAMPOS OBLIGATORIOS COMPLETOS
    // =========================
    const r = await request(settings, 'POST', 'products', {
        ref: safeSku,
        label: product.name,
        ...
        price: price,
        price_base_type: 'HT',
        tva_tx: 21.0,        // 💡 OBLIGATORIO: Asegura el tipo de IVA en Dolibarr
        type: 0,             // 💡 OBLIGATORIO: 0 significa Producto
        status: 1,           // 💡 OBLIGATORIO: 1 = En venta
        status_buy: 1,       // 💡 OBLIGATORIO: 1 = En compra
    });

    if (!r.ok) {
        console.error(`[DOLIBARR CREATION FAIL] SKU: ${safeSku} | Code: ${r.code}`, r.data);
        return {
            ok: false,
            error: r.error || `Error creando producto en ERP (HTTP ${r.code})`
        };
    }

    // Extraemos el ID tanto si viene directo como si viene dentro de un objeto devuelto
    const erpId = typeof r.data === 'object' ? (r.data.id || r.data.rowid) : r.data;
