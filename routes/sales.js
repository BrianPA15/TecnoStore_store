...
router.post('/', apiTokenMiddleware('write:products'), (req, res) => {
    ...
    // evitar duplicados
    db.get('SELECT 1 FROM sales WHERE order_id = ? LIMIT 1', [order_id], (err, existing) => {
        if (existing) {
            return res.status(409).json({ error: 'Pedido ya registrado.' });
        }
        ...
        db.serialize(() => {
            ...
            if (hasError) {
                db.run('ROLLBACK');
                return res.status(400).json({ error: 'Error en los datos de la venta.' });
            }

            db.run('COMMIT', (err) => {
                if (err) {
                    db.run('ROLLBACK');
                    return res.status(500).json({ error: 'Error procesando venta.' });
                }
...
// ================= ORDER DETAIL =================
router.get('/order/:order_id', authMiddleware, adminMiddleware, (req, res) => {
    db.all(
        'SELECT * FROM sales WHERE order_id = ? ORDER BY id ASC',
        [req.params.order_id],
        (err, rows) => {
